...
# on all EVEN reloads of the element by ajax calls. Weird.
    ...
    my $pad = $self->render_image_pad(
        $args{section}
            || Bio::Graphics::Browser2::Render->get_section_from_label(
            $label),
    );
    ...
    my $subtrack_labels = join '', map {
        my ( $label, $left, $top ) = @$_;
        div({   -class => 'subtrack',
                -style => "top:${top}px;left:20px;background-color:white"
            },
            $label
        );
    } @$titles;
...
sub make_map {
    ...
    foreach my $box (@$boxes) {
        ...
        my $title = unescape(
            $self->make_title( $box->[0], $panel, $label, $box->[5] ) );
        my $target
            = $self->make_link_target( $box->[0], $panel, $label, $box->[5] );
        ...
        if ($tips) {
            ...
            $balloon_ht ||= $source->global_setting('balloon style')
                || 'GBubble';
...
sub add_features_to_track {
    ...
    for my $db ( keys %db2db ) {
        ...
        for my $l (@labels) {
            ...
            if ($source->show_summary(
                    $l, $self->vis_length, $self->settings
                )
                )
            {
                $is_summary{$l}++;
...
# the latter is traditional name-based grouping based on a common prefix/suffix
    ...
    for my $l ( keys %$tracks ) {
        ...
        {    # may not be present in older Bio::Graphics
            ...
            $tracks->{$l}->configure(
                -key => $self->language->translate(
                    'FEATURES_CLIPPED', $max, $count
                )
            );
        }
    }
...
sub create_track_args {
    ...
    my $is_summary
        = $source->show_summary( $label, $self->vis_length, $self->settings );
    ...
    if ($is_summary) {
        ...
        push @override, $source->i18n_style( "$label:summary", $lang )
            if $source->Bio::Graphics::FeatureFile::setting("$label:summary");
    }
    ...
    if ( $source->semantic_setting( $label => 'global feature', $length ) ) {
        ...
        @args = (
            ...
            $source->i18n_style( $label, $lang ), @override,
        );
    }
    else {
        @args = (
            @default_args, $source->default_style,
            $source->i18n_style( $label, $lang, $length ), @override,
        );
    }
    return @args;
}
