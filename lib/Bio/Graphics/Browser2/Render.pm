...
use CGI
    qw(:standard param request_method header url iframe img span div br center url_param);
...
sub run {
    ...
    warn "[$$] RUN(): ",
        request_method(), ': ', url( -path => 1 ), ' ', query_string()
        if $debug || TRACE_RUN;
...
sub background_track_render {
    ...
    {
        $requests{'detail'} = $self->render_deferred(
    ...
    $requests{'region'} = $self->render_deferred(
    ...
    $requests{'overview'} = $self->render_deferred(
...
sub state_cookie {
    ...
    my $cookie  = CGI::Cookie->new(
        ...
        -path    => $path,
        -expires => '+'
            . $globals->time2sec( $globals->remember_settings_time ) . 's',
    );
    return $cookie;
}
...
sub render_body {
    ...
    $output .= $self->render_tabbed_pages( $main_page, $tracks, $community,
        $custom, $global_config );
...
sub handle_gff_dump {
    ...
    if ( $actions{scan} ) {
        ...
        if ( $actions{save} && ( $actions{datafile} || $actions{gff3} ) ) {
            ...
            print header(
                -type => $mime =~ /x-/ ? 'application/x-fasta' : $mime,
...
sub handle_track_dump {
    ...
    if ( my @labels
        = $source->track_source_to_label( shellwords param('ts') ) )
    {
        %ts = map { $_ => 1 } @labels;
    }
    if ( my @labels
        = $source->data_source_to_label( shellwords param('ds') ) )
    {
        %ds = map { $_ => 1 } @labels;
    }
...
sub handle_plugins {
    ...
    warn
        "an operation was requested on plugin $id, but this plugin has not been configured"
        unless $plugin;
...
sub update_state {
    ...
    return
        if param('gbgff')
    ;    # don't let gbgff requests update our coordinates!!!
    $self->_update_state;
}
...
sub update_coordinates {
    ...
    if ( param('ref') ) {
        $state->{ref}        = param('ref');
        $state->{view_start} = param('start')
            if defined param('start') && param('start') =~ /^[\d-]+/;
        $state->{view_stop} = param('stop')
            if defined param('stop') && param('stop') =~ /^[\d-]+/;
        $state->{view_stop} = param('end')
        ...
        unless ( defined $state->{ref} ) {
            ...
            if ( $state->{backup_region} ) {    # last known working region
                ...
                $state->{name} 
                    ...
                    || param('q')
                    || url_param('name')
                    || url_param('q');          # get the region somehow!!
                if ( my $seg = $self->segment ) {
                    $state->{ref}   = $seg->seq_id;
...
sub bookmark_link {
    ...
    $q->param( -name => 'id', -value => $settings->{userid} )
...
sub image_link {
    ...
    my $options  = join '+',
        map { join '+', CGI::escape($_), $settings->{features}{$_}{options} }
        map { /\s/ ? "$_" : $_ }
...
sub fcgi_request {
    ...
    my $request = FCGI::Request( \*STDIN, \*STDOUT, \*STDERR, \%ENV, 0,
